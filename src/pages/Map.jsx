...
function Map({ logo }) {
  ...
  const api =
    "https://raw.githubusercontent.com/itsdaiyy/Hex-React-project-data/refs/heads/main";
  ...
  const customIcon = new L.Icon({
    iconUrl:
      "https://as1.ftcdn.net/jpg/05/23/83/42/480_F_523834217_W7Tz83KoT9yoa4HH9AdqNtYAYrE0qMEy.png?token=1737953942_Xo2cx3OUeyt1ehBh_2i36tB_3VqF2qIRLzkH6QzvRAU",
    iconSize: [50, 50],
  });

  const countRecyclableBoxes = (item) => {
    return (
      item.recyclableBoxes.XL +
      item.recyclableBoxes.L +
      item.recyclableBoxes.M +
      item.recyclableBoxes.S
    );
  };
      ...
      <div className='py-5 px-5 xl:container mx-auto flex flex-col md:flex-row justify-between'>
        {activeStation ? (
          <div className='w-5/12 px-5 flex-shrink-0 flex-grow-0'>
            ...
            <div className='flex gap-3 mb-5'>
              <div className='w-4/12'>
                <img
                  ...
                  alt={activeStation.stationName}
                />
              </div>
              <div className='w-8/12'>
                <div>
                  <h5 className='text-base flex gap-2 mb-1'>
                    {countRecyclableBoxes(activeStation) ? (
                      <span className='rounded-lg bg-blue-600 text-white p-2 text-sm'>
                        可認領
                      </span>
                    ) : (
                      <></>
                    )}
                    {countPendingBoxes(activeStation) ? (
                      <span className='rounded-lg bg-green-600 text-white p-2 text-sm'>
                        可認領
                      </span>
                    ) : (
                      <></>
                  ...
                  <p className='text-base'>
                    ...
                    <br />
                    電話：
                    {activeStation.phone ? activeStation.phone : "尚未填寫"}
                    <br />
                    營業時間：{getStationDailyHours(activeStation)}
                  </p>
                </div>
              </div>
            </div>
            <h6 className='rounded-lg bg-gray-600 text-white p-2 text-sm text-center mb-3'>
              回收認領資訊
            </h6>
            ...
            <div className='w-12/12 flex justify-center'>
              <Link
                to='/'
                className='rounded-lg bg-yellow-600 text-white p-2 text-sm'
              >
                查看紙箱列表
              </Link>
            </div>
        ...
        >
          ...
          {stations.map((item) => (
            ...
            >
              <Popup>
                <h5 className='font-bold text-xl text-center mb-3'>
                  {item.stationName}
                </h5>
                <div className='flex gap-2 justify-center'>
                  <button
                    type='button'
                    className='rounded-lg bg-blue-600 text-white py-2 px-3'
                  >
                    可回收 {countRecyclableBoxes(item)} 個
                  </button>
                  <button
                    type='button'
                    className='rounded-lg bg-green-600 text-white py-2 px-3'
                  >
                    可認領 {countPendingBoxes(item)} 個
                  </button>
                </div>
                ...
                <div className='flex justify-center'>
                  <button
                    type='button'
                    className='rounded-lg bg-gray-600 text-white py-2 px-3'
                  >
                    查看紙箱列表
                  </button>
                </div>
